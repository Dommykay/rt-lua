...
function Camera()
    ...
    camera.renderpixel = function (x, y, r, g, b, a)
        if x == 0 then print(string.format("%.2f%% DONE",y/camera.y * 100)) end
        if x%camera.upscaling == 0 and y%camera.upscaling == 0 then
    ...
    camera.nearestneighbor = function(x, y, r, g, b, a)
        if x%camera.upscaling ~= 0 or y%camera.upscaling ~= 0 then
            r,g,b,a = camera.buffer:getPixel(math.floor(math.max(x,1)/camera.upscaling)*camera.upscaling, math.floor(math.max(y,1)/camera.upscaling)*camera.upscaling)
        end
        return r,g,b,a
    ...
    camera.initialize = function ()
        camera.noisereduction = 1
        camera.samples = 1
        camera.bouncedepth = 75
        camera.upscaling = 1
    ...
    camera.samplerange = function ()
        return instancevec3((random()-0.5)*camera.upscaling,(random()-0.5)*camera.upscaling,0)
    end
