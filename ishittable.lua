function HittableList()
    ...
    hittablelist.hits = function (ray, distmin, distmax, hitmemory)
        local temporary_memory = HitMemory()
        local hit_anything = false
        local closest_so_far = distmax

        for object in hittablelist.list do
            local hit, temporary_memory = object.hit(ray, distmin, closest_so_far, temporary_memory)
            if hit then
                hit_anything = true
                closest_so_far = temporary_memory.t
                hitmemory = temporary_memory
            end
        end
        return hit_anything, hitmemory
    end
end
...
function ReturnHittable()
    local hittable = {}
    -- hittable.hit = function (ray, distmin, distmax, hitmemory) return 0 end
    return hittable
end
